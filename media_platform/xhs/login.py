...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def check_login_state(self, no_logged_in_session: str) -> bool:
        """
            ...
            retry decorator will retry 900 times if the return value is False, and the retry interval is 1 second (15 minutes total)
    ...
    async def login_by_qrcode(self):
        ...
        utils.logger.info(f"[XiaoHongShuLogin.login_by_qrcode] waiting for scan code login, remaining time is 900s (15 minutes)")
        try:
            await self.check_login_state(no_logged_in_session)
        except RetryError:
