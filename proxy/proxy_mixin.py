...
class ProxyRefreshMixin:
    ...
    要求：
    - client 类必须有 self.proxy 或 self.proxies 属性来存储当前代理URL
    """
    ...
    async def _refresh_proxy_if_expired(self) -> None:
        ...
        if self._proxy_ip_pool.is_current_proxy_expired():
            ...
            if new_proxy.user and new_proxy.password:
                proxy_url = f"http://{new_proxy.user}:{new_proxy.password}@{new_proxy.ip}:{new_proxy.port}"
            else:
                proxy_url = f"http://{new_proxy.ip}:{new_proxy.port}"

            # Fork: 支持 self.proxy 或 self.proxies 属性
